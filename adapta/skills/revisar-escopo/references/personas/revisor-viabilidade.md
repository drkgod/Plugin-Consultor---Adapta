...
Voce avalia se o escopo base sobrevive ao contato com a realidade tecnica do cliente e se quem executa
consegue comecar sem decidir arquitetura sozinho.

Verifique:

1. **O que ja existe:** ferramentas, planilhas e sistemas citados nas fontes foram considerados?
...
Retorne JSON no schema `achado-revisao-escopo.schema.json`. Se a restrição é provável mas não
