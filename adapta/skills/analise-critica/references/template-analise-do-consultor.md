# Análise do consultor

> Este arquivo é do consultor humano. A IA cria a estrutura e mantém as decisões em aberto; não
> preenche concordância, discordância ou escolha em nome dele.

## Resposta aos achados

### AC-001 — [título do achado]

- **Minha leitura:** [concordo | discordo | concordo parcialmente]
- **Evidência ou contexto que acrescento:**
- **Decisão para o escopo final:**
- **Razão da decisão:**

_Repita um bloco para cada achado e decisão humana da análise crítica._

## Novas ideias do consultor

| ID | Ideia | Evidência/hipótese | Valor esperado | Risco/custo | Levar ao escopo? |
|---|---|---|---|---|---|

## Riscos ou pontos de vista que a análise não cobriu

-

## Mudanças que quero levar ao escopo final

| Mudança | O que substitui | Por quê | Fase afetada |
|---|---|---|---|

## Checkpoint humano

- [ ] Respondi todos os IDs obrigatórios.
- [ ] Registrei discordâncias e novas evidências.
- [ ] Diferenciei mudança do ciclo atual de ideia para o próximo ciclo.
- [ ] Autorizo usar estes apontamentos como input de `/adapta:escopo-definitivo`.

