...
description: Consolida o escopo base, a análise crítica e a autoria humana em 03-Projeto/02-Escopo-Definitivo.md, exatamente cinco fases e o plano de ação atual, com revisão documental multipersona antes dos gates de liberação.
---
...
`../../contracts/consultor-workflows.json`. Geração e revisão documental são um único job.
...
3. Escreva um único `03-Projeto/02-Escopo-Definitivo.md` com resultado de negócio, atores, fluxo,
   ...
   decisões do consultor. O antigo conjunto `PRD.md` + `escopo.md` deixa de ser criado.
...
5. Preserve objetivo mensurável, fora de escopo, riscos, sequência ASA, entrega visível,
   checklist e critério de aceite por fase. Fases futuras precisam chegar às SPECs sem decisão
...
8. Crie `.adapta/checks/check-escopo.md` como controle interno `PENDENTE`; a skill não o aprova.
9. Atualize `STATUS.md` e `changelog.md`.
